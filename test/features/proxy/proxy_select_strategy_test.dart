...
void main() {
  group('ProxyCountries', () {
    test('has complete unique ISO list', () {
      expect(ProxyCountries.all.length, 250);
    ...
    test('popular countries appear first in picker order', () {
      ...
      expect(sorted.map((country) => country.code).toSet(), ProxyCountries.codes);
      expect(sorted.length, ProxyCountries.all.length);
  ...
  group('isAlive', () {
    test('rejects groups, zero delay and timeout', () {
      ...
      expect(ProxySelectAlgorithm.isAlive(_proxy(tag: 'a', delay: 64999)), isTrue);
      expect(ProxySelectAlgorithm.isAlive(_proxy(tag: 'a', delay: 65000)), isFalse);
    });
  });
